...
cudaError_t allocPrimitive(void** ptr, size_t size, AllocParams& p) {
  printf("allocPrimitive: %d %d\n", int(p.pool->owner_PrivatePool && p.pool->owner_PrivatePool->allocator()), int(size));
  if (p.pool->owner_PrivatePool && p.pool->owner_PrivatePool->allocator()) {
    *ptr = p.pool->owner_PrivatePool->allocator()->raw_alloc(size);
    printf("allocPrimitive: %p %d\n", *ptr, int(size));
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  Block* alloc_found_block(
    ...
    printf("alloc_found_block: %p %p %p %p %p %p %d\n", block->prev, block, block->next, (block->prev ? block->prev->ptr : nullptr), block->ptr, (block->next ? block->next->ptr : nullptr), int(size));
    return block;
  }
  ...
  void free(Block* block) {
    ...
    printf("free(block): %p %p %p %p %p %p %d %d %d\n", block->prev, block, block->next, (block->prev ? block->prev->ptr : nullptr), block->ptr, (block->next ? block->next->ptr : nullptr), int(block->size), int(block->stream_uses.size()), int(captures_underway.size()));
  ...
  void beginAllocateToPool(
    ...
    printf("beginAllocateToPool: %llu %llu %zu\n", mempool_id.first, mempool_id.second, captures_underway.size());
  }
  ...
  void endAllocateToPool(MempoolId_t mempool_id) {
    ...
    printf("endAllocateToPool: %llu %llu %zu\n", mempool_id.first, mempool_id.second, captures_underway.size());
  ...
  void releasePool(MempoolId_t mempool_id) {
    ...
    printf("releasePool: %llu %llu %d\n", mempool_id.first, mempool_id.second, uc);
 ...
 private:
  ...
  void create_or_incref_pool(
    ...
    printf("create_or_incref_pool: %llu %llu %zu\n", mempool_id.first, mempool_id.second, graph_pools.size());
  }
  ...
  void free_block(
    ...
    printf("free_block: %p %p %p %p %p %p %d %d %d %d\n",
      block->prev, block, block->next, (block->prev ? block->prev->ptr : nullptr),
      block->ptr, (block->next ? block->next->ptr : nullptr), int(block->size),
      int(block->is_split()), int(net_change_inactive_split_blocks), int(net_change_inactive_split_size));
  ...
  bool alloc_block(
    ...
    printf("alloc_block: %d %d\n", int(active_pool), int(size));
    if (allowed_memory_maximum.has_value() &&
  ...
  bool release_cached_blocks(
      const std::shared_ptr<GatheredContext>& context,
      MempoolId_t mempool_id) {
    printf("release_cached_blocks: %llu %llu %zu\n", mempool_id.first, mempool_id.second, graph_pools_freeable.size());
    if (mempool_id.first == 0 && mempool_id.second == 0 &&
    ...
    for (auto it = graph_pools_freeable.begin();
      ...
      printf("release_cached_blocks: %llu %llu %d %d\n", it->first.first, it->first.second, it->second->use_count, it->second->cudaMalloc_count);
  ...
  void release_block(
    ...
    auto* pool = block->pool;
    printf("release_block: %p %llu %llu %p %p\n",
      pool, pool->owner_MempoolId().first, pool->owner_MempoolId().second, pool->owner_PrivatePool,
      (void*)(pool->owner_PrivatePool ? pool->owner_PrivatePool->allocator() : nullptr));
  ...
  void release_blocks(
    ...
    while (it != pool.blocks.end()) {
      Block* block = *it;
      printf("release_blocks: %p %p %p %p %p\n", block->prev, block, block->next, (block->prev ? block->prev->ptr : nullptr), block->ptr);
      ++it;
      if (block->expandable_segment_) {
