#include <c10/cuda/CUDACustomizableAllocator.h>
...
namespace cuda::CUDACustomizableAllocator {
...
struct AllocatorConfig {
  bool uses_expandable_segments;
  bool uses_peer_access;
  bool uses_caching;
  int provided_alignment;
};
...
class CustomAllocator {
  public:
    virtual AllocationParams alloc(size_t nbytes, cudaStream_t stream) = 0;
    virtual void free(AllocationParams params, cudaStream_t stream) = 0;
    virtual void map(void* ptr, size_t nbytes, cudaStream_t stream) {
      TORCH_CHECK_NOT_IMPLEMENTED(
        config.uses_expandable_segments, "allocators using expandable segments must implement map.");
    }
    virtual void unmap(void* ptr, size_t nbytes, cudaStream_t stream) {
      TORCH_CHECK_NOT_IMPLEMENTED(
        config.uses_expandable_segments, "allocators using expandable segments must implement unmap.");
    }
 ...
 public:
  ...
  Block* alloc_found_block(
    ...
    auto allocated_bytes_gauge =
        STATIC_GAUGE(pytorch.CUDACustomizableAllocator.allocated_bytes);
  ...
  std::vector<SegmentInfo> snapshot(MempoolId_t mempool_id) {
    ...
    if (mempool_id.first != 0 || mempool_id.second != 0) {
      ...
      // all the blocks in the CUDACustomizableAllocator (as returned by
      // get_all_blocks).
 ...
 private:
  ...
  bool map_block(
    ...
    auto reserved_bytes_gauge =
        STATIC_GAUGE(pytorch.CUDACustomizableAllocator.reserved_bytes);
  ...
  void free_block(
    ...
    if (pool.owner_PrivatePool) {
      // for graph pools, if we do not have any active blocks left,
      // release all cached blocks from the pool
